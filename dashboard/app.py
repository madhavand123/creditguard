...
from risk_grading import assign_grade
...
with tab_score:
    ...
    with col_result:
        if submitted:
            ...
            # Grade — same function the API uses, so the dashboard and the
            # API can never disagree on a borrower's grade.
            grade_info = assign_grade(prob)
            grade, label, color = grade_info["grade"], grade_info["label"], grade_info["color"]
            ...
            st.markdown(
                ...
                f"<div style='font-size:18px;font-weight:500;color:{color}'>{label}</div>"
